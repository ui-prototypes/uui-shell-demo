...
/*
  ...
  - _fields is an array of object describing data fields from the backend
       in the native JQWidgets format
...
*/
var TableJQXPageInfo = PageInfo.extend(
{
	construct: function(htmlFile, componentId, title,
			tableId, tableDataUrl, fields, columns,
			keyDataField, parentDataField, buttons, initFn)
	{
		...
		this._keyDataField = keyDataField;
		this._parentDataField = parentDataField;
	...
	getName: function()
	{
		...
		return "TableJQXPageInfo(" + this.getId() + ") extends " +
				arguments.callee.$.getName.call(this);
	},
	loadPageFromObject: function(keyData) // "this" is TableJQXPageInfo and optional dictionary object
	{
		var info = this;
		...
		$('#workArea').load(info._htmlFile + ' #' + info._componentId, function(response, status, xhr)
		{
			if (info._tableId != null)
			{
				...
				var source =
				{
					...
					dataFields: info._fields,
					hierarchy:
					{
						keyDataField: { name: info._keyDataField },
						parentDataField: { name: info._parentDataField }
					},
					id: info._keyDataField,
					url: info._tableDataUrl
				};
				var dataAdapter = new $.jqx.dataAdapter(source);
				info._$table = $('#' + info._tableId).jqxTreeGrid(
				{
					width: '100%',
					height: '100%',
					source: dataAdapter,
				...
				$('#' + info._tableId).on('rowSelect', function(event)
				{
					// event arguments
					//var args = event.args;
					// row data
					...
					// row key
					//var rowKey = args.key;
					var selectedRows = $(info._$table).jqxTreeGrid('getSelection');
					...
					for (var i = 0; i < selectedRows.length; i++)
					{
						...
						var dataItem =
						{
							rowIndex: selectedRow[info._keyDataField],
							rowData: selectedRow
						};
	...
	{
		// event.data is the TableJQXPageInfo object that manages the table
		var info = event.data;
		if (info == null) { return; }
		if (!(info instanceof TableJQXPageInfo)) { return; }
		info.refreshTable();
		//closeSlideUp();
	},
...
var TopClass = TableJQXPageInfo.extend(
{
	getName: function()
	{
		// calls the getName() method of TableJQXPageInfo
...
// alerts "TopClass(2) extends TableJQXPageInfo(2) extends PageInfo(2)"
// looks good again, and there's no intermediate functions!
/*alert(new TopClass().getName());
alert(new TableJQXPageInfo().getName());*/
