...
/*
 *********** THIS OBSOLETE VERSION USED THE ORIGINAL DATATABLE PLUGIN
  TablePageInfo members:
...
*/
var TablePageInfo = PageInfo.extend(
{
	construct: function(htmlFile, componentId, title,
		dashboardId, dashletInfo, tableId, tableDataUrl, buttons, initFn)
	{
	...
	getName: function()
	{
		...
		return "TablePageInfo(" + this.getId() + ") extends " +
				arguments.callee.$.getName.call(this);
	},
	loadPageFromObject: function(keyData) // "this" is TablePageInfo and optional dictionary object
	{
		var info = this;
		...
		$('#workArea').load(info._htmlFile + ' #' + info._componentId, function(response, status, xhr)
		{
			if (info._tableId != null)
			{
				...
				info._$table = $('#' + info._tableId).dataTable(
				{
					"bProcessing": true,
					"bServerSide": false,
					"sServerMethod": "POST",
					"sAjaxSource": info._tableDataUrl + info._latestQueryString,
					"fnServerData": function(sSource, aoData, fnCallback)
					{
						$.getJSON(sSource, aoData, function(json)
						{
							/* save the fat data for later reference */
							info._latestFetchedData = json.aaData;
							...
							var skinnyData = info._latestFetchedRowData;
							fnCallback( { "aaData": skinnyData } );
						} );
					},
					"fnInitComplete": function(oSettings, json)
					{
						/*** NOT USED *** if (info._initialSort != null)
						{
							info._$table.fnSort( [ info._initialSort ] );
						}*/
						$(info._$table.fnGetNodes()).bind('click', info._$table, info.onClickTableRow);
						$(info._$table.fnGetNodes()).bind('click', info, info.onSelectedTableRow);
	...
	{
		// event.data is the TablePageInfo object that manages the table
		var info = event.data;
		if (info == null) { return; }
		if (!(info instanceof TablePageInfo)) { return; }
		info.refreshTable();
		closeSlideUp();
	},
...
var TopClass = TablePageInfo.extend(
{
	getName: function()
	{
		// calls the getName() method of TablePageInfo
...
alert(new TablePageInfo().getName());*/
